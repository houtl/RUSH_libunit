...
/*   Updated: 2016/11/23 11:53:45 by atrudel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		read_from_buffer(t_buffer *buffer, char **line)
{
	...
	free(*line);
	*line = to_add;
	if (output == 10 && ft_strlen(*line) == 0 && buffer->eof == 1)
		output = 0;
	buffer->curseur = i + 1;
	if (output == 10 && i == buffer->end && buffer->eof == 1)
		output = 1;
	return (output);
}
...
int				get_next_line(const int fd, char **line)
{
	...
	if (!(buffer = get_buffer(&buffer_list, fd)))
		return (-1);
	if (buffer->curseur >= buffer->end && buffer->eof == 1)
	{
		*line = ft_strnew(1);
